...
%
%   HeaderClickedCallback - callback when header is clicked (any button)
%       Callback function passed:
%           obj = handle for table object
%           event = struct
%                       .Position = [x,y] position in pixels of click
%                       .Location = 'header'
%                       .Cell = [row,col] of cell below cursor (note row=-1)
%                       .JavaMouseEvent = copy of java MouseEvent class
%                       .Button = (1,2,3...) mouse button pressed
%           Note: the callback is called even if a context menu has been
%           assigned to the header
%
%   HeaderUIContextMenu = [] assigne a context menu created by
%                         uicontextmenu().
%           Note: the context menu is drawn before the user supplied
...
%          'single' - only one row at a time (default)
%          'contiguous' - multiple contiguous rows
%           'discontiguous' - any row at the same time
%
...
%
%% Auth/Revision:
%	2016-08-05 Daniel T. Kovari, Emory University
%		Added the ability to specify a callback when the user clicks the header
%		You can also assicoate a uicontextmenu with the header.
%		Currently when a right-click is executed on the header, the specified context menu
%		is displayed, then the specified callback is fired.
%		Callback is passed an event structure that lists the button pressed
%
...
% the thread-safe EDT.
    ...
    methods
        function obj = Table(varargin)
            ...
            p.addParamValue('ColumnResizePolicy','AUTO_RESIZE_OFF');
    ...
    methods (Access = private)
        function create(obj, ParamsToSet)
            ...
            jT_header = handle(jTable.getTableHeader(),'callbackproperties');
            %allfields = sortrows(fields(set(jT_header)))

            fn = @(~,e) onHeaderClick(obj,e,'header');
            jT_header.MouseClickedCallback = fn;
            
            %jT_header.MouseClickedCallback = fn;
            %jT_header.tableHeader()
            
            %set(jT_header,'MouseClickedCalback',@(~,evt) disp(evt))
            ...
            jScrollPane = javaObjectEDT('javax.swing.JScrollPane', jTable,javax.swing.JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,javax.swing.JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    ...
    methods
        ...
        function set.HeaderUIContextMenu(obj,value)
            if ~isempty(value) && ( ~ishghandle(value) || ~isscalar(value) || ...
                    ~strcmp(get(value, 'Type'), 'uicontextmenu') )
                error('HeaderContextMenu must be handle to context menu.');
            end
            % Set
            obj.HeaderUIContextMenu = value;
        end
        % Units
    ...
    methods(Access = private)
        ...
        function onMouseClick(obj,e,location)
            ...
            if ~callbacksEnabled(obj), return; end
            ...
            if e.isMetaDown
                ...
                if row>0 && ~any(obj.SelectedRows == row)
        ...
        function onHeaderClick(obj,e,location) %click on header
            if obj.Debug
                disp('onHeaderClick');
                disp(location);
                disp(e);
            ...
            if callbacksEnabled(obj)
                ...
                col = obj.JTable.columnAtPoint(e.getPoint) + 1;
                obj.ClickColumn = col;
                if e.isMetaDown %right click - open context menu if specified
                    CMenu = obj.HeaderUIContextMenu;
                    if ~isempty(CMenu)
                        tPos = getpixelposition(obj.Container,true);
                        mPos = [x+tPos(1) tPos(2)+tPos(4)-y+obj.JScrollPane.getVerticalScrollBar().getValue()];
                        set(CMenu,'Position',mPos,'Visible','on');
                ...
                if ~isempty(obj.HeaderClickedCallback)
                    event = struct(...
                        'Position',[x,y],...
                        'Location',location,...
                        'Cell',[-1,col],...
                        'JavaMouseEvent',e,...
                        'Button',e.getButton());
                    hgfeval(obj.HeaderClickedCallback,obj,event);
                end
